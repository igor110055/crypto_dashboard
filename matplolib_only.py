...
def animate(i):
    ...
    a.cla()
    a.plot(data.index, data.Close, 'b')
    a2.cla()
    a2.plot(data.index, data.Volume, 'r')
...
class StartPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        button1 = ttk.Button(self, text="BTC Chart", command=lambda: [controller.show_frame(BTC_Page), changeAsset('BTCUSDT')])
        button1.pack()

        button2 = ttk.Button(self, text="ETH Chart", command=lambda: [controller.show_frame(ETH_Page), changeAsset('ETHUSDT')])
        button2.pack()
